        ...
        <section ref={containerRef} className="w-full bg-background py-20 px-6 lg:px-24 flex flex-col justify-center relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto w-full z-10 relative perspective-[1000px]">
                {categories.map((category, idx) => (
                    <div key={idx} className="tech-card bg-black/20 border border-white/10 rounded-2xl p-8 hover:bg-black/40 hover:border-white/20 transition-all duration-300 group">
                        <h3 className="text-2xl font-bold text-white mb-6 group-hover:text-secondary transition-colors">{category.title}</h3>
                        <div className="flex flex-wrap gap-2">
                            {category.items.map(item => (
                                <span key={item} className="px-3 py-1.5 text-sm font-medium text-gray-300 bg-white/5 border border-white/5 rounded-full backdrop-blur-sm group-hover:border-white/10 transition-colors">
                                    {item}
                                </span>
                            ))}
                        </div>
